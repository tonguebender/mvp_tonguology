...
function getDefinition(id) {
  return httpTransport.get(getDefinitionUrl(id)).then(data => {
    if (data) {
      const meanings = data.data.meanings.map(meaning => `_(${meaning.speech_part})_ ${meaning.def}`).join(';\n');
...
function getExamples(id) {
  return httpTransport.get(getExamplesUrl(id)).then(
    data => {
...
function getSynonyms(id) {
  return httpTransport.get(getSynonymsUrl(id)).then(data => {
    if (data) {
...
function getIPA(id) {
  return httpTransport.get(getIPAUrl(id)).then(data => {
    if (data) {
...
function getTranslation(id) {
  return httpTransport.get(getTranslationUrl(id)).then(data => {
    if (data) {
      return `*${id}*: ${data.join(', ')}`;
    }
  });
