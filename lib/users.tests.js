...
const tests = [
  ...
  () => {
    return 'skip';
    return users.addTaskOnTop('user2', {type: 'wqewqw', options: {bla:1}})
      .then(res => {
  ...
  () => {
    return users.create('user1', { name: 'bla', chatId: '12312', fullInfo: {bla:1} })
      .then(res => {
        console.log('users.create() result:', res);

        return users.addSubscription('user1', 'course1')
          .then(res => {
