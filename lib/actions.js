...
function processAction(action) {
  switch (action.action) {
    ...
    case ACTIONS.USER_COMMAND_STOP: {
      return validateRequired(['userId'], action)
        .then(action => {
          ...
          if (text) {
            return users.getSubscriptions(userId)
              .then(subscriptions => {
    ...
    case ACTIONS.RESPOND_WITH_DEFINITION: {
      return validateRequired(['userId', 'text'], action)
        .then(action => {
          ...
          return tongues.getDefinition(text)
            .then(definition => {
              if (definition) {
    ...
    case ACTIONS.RESPOND_WITH_EXAMPLES: {
      return validateRequired(['userId', 'text'], action)
        .then(action => {
          ...
          return tongues.getExamples(text)
            .then(examples => {
              return [
                examples
                  ? {
    ...
    case ACTIONS.RESPOND_WITH_SYNONYMS: {
      return validateRequired(['userId', 'text'], action)
        .then(action => {
          ...
          return tongues.getSynonyms(text)
            .then(synonyms => {
              return [
                synonyms
                  ? {
    ...
    case ACTIONS.RESPOND_WITH_TRANSLATION: {
      return validateRequired(['userId', 'text'], action)
        .then(action => {
          ...
          return tongues.getTranslation(text)
            .then(translation => {
              return [
                translation
                  ? {
    ...
    case ACTIONS.RESPOND_WITH_QUIZ_ANSWER: {
      return validateRequired(['userId', 'contextData'], action)
        .then(action => {
          ...
          return [
            {
              action: ACTIONS.SEND_MESSAGE,
              userId,
              text: `Available courses:\n${courses.map((course, i) => `${i + 1}. ${course}`).join(' ')}.\nChoose a number.`
            },
            {
              action: ACTIONS.ADD_TASKS_ON_TOP,
              userId,
            ...
            return [
              {
                action: ACTIONS.SEND_MESSAGE,
                userId,
                text: `Your courses:\n${subscriptions.map((course, i) => `${i + 1}. ${course}`)}.\nTo stop a subscription type: stop 1.`
              },
            ];
          } else {
            return [
    ...
    case ACTIONS.ADD_TASKS_ON_TOP: {
      return validateRequired(['userId', 'tasks'], action)
        .then(action => {
...
function userTextToAction(text, userId) {
  return users.getTopWaitTask(userId)
    .then(waitTask => {
