...
db.getConnection = function getConnection() {
  if (db._connectionPromise) {
    ...
    return db._connectionPromise = new Promise((resolve, reject) => {
      mongoose.connect('mongodb://localhost:27017/tonguology', { useMongoClient: true }, (err, connection) => {
        if (err) {
...
db.get = function get(collection, id) {
  if (typeof collection !== 'string') throw new Error(`collection must be string, instead: ${typeof collection}`);
...
db.findAll = function getAll(collection) {
  return new Promise((resolve, reject) => {
    db._getModel(collection)
      .then(Model => {
        Model.find({}, (err, docs) => {
